...
;
        ...
        public void PayRent(Building bulding, List<User> user, Field field, int firstCube, int secondCube)
        {
            ...
            if (((Business)bulding).Rent[((Business)bulding).Level] < Balance && Balance >= summa)
            {
            ...
            {    
                        ...
                        for (int i = 0; i < user.Count; i++)
                        {
                            if (((Business)bulding).BusinessOwner == user[i].Symbol)
                            {
                                if (bulding.GetType() == typeof(GamingCompanies))
                                {
                                    ...
                                    Console.WriteLine($"Игрок {Symbol} выплатил ренту игроку {user[i].Symbol} цена {Balance}");
                                    Thread.Sleep(2000);
                                    return;
                                }
                                else
                ...
                while (((Business)bulding).Rent[((Business)bulding).Level] >= Balance && Balance >= summa);
                if (((Business)bulding).Mortgaged == false)
                {
                    if (bulding.GetType() == typeof(GamingCompanies))
                    {
        ...
        public int CountBsnPrice(Business business, List<Building> buildings, List<User> users, char symbol)//проверка сколько бизнесов типа у бота цена 
        {
            double summa = 0;
            int countBsn = 0;
            int countBsnEnemy = 0;
        ...
        public bool MortagagedBsnOtherType(Business business, List<Building> Mybuildings, List<Building> Allbuildings, List<User> users)//метод закладывает бизнес для покупки бизнеса на который попал игрок в случаи нехватки деняг 
        {
            ...
            //    {
            //        if (((Business)Mybuildings[i]).BusinessType != business.BusinessType && ((Business)Mybuildings[i]).BusinessOwner == Symbol)
            //        {
            ...
            for (int i = 0; i < Mybuildings.Count; i++)
            {
                ...
                {
                    max = ((Business)Mybuildings[i]).RansomValue;
                }
